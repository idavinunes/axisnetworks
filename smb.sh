...
Titulo:                : Samba4 4.10.4
Description            : Samba4 Instalador Automatizado
...
#########################################################################################
#                                                                                       #
# * Deseja personalização ou alterações no instalador para atender sua demanda ? *      #
#                                                                                       #
...
yum install -y ftp://ftp.pbone.net/mirror/ftp.centos.org/7.6.1810/storage/x86_64/ceph-jewel/lttng-ust-2.10.0-1.el7.x86_64.rpm
clear
$ARTE
echo "VAMOS AGORA BAIXAR COMPILAR O SAMBA 4.10.4 (VA TOMAR UM CAFEZINHO...)"
sleep 7s
...
cd samba-4.10.4
sed -i "s/krb5-config.heimdal/heimdal-krb5-config/" source4/heimdal_build/wscript_configure
sed -i "s/krb5-config.heimdal/heimdal-krb5-config/" wscript_configure_system_mitkrb5
...
find /usr/lib/samba -type d > /etc/ld.so.conf.d/samba.conf
ldconfig
ldconfig
ldconfig
ldconfig
...
#sa3mba-tool domain passwordsettings set --min-pwd-age=0
...
clear
$ARTE
mkdir /var/lib/samba/sysvol/"$REALM"/scripts/MILVUS
mkdir /var/lib/samba/sysvol/"$REALM"/scripts/RMM
sleep 5s
clear
echo -n > /etc/samba/smb.conf
cat >> /etc/samba/smb.conf <<EOF
...
clear
$ARTE
read -r -p  "INSTALAÇÃO FINALIZADA COM SUCESSO APERTE <ENTER> PARA REINICIALIZAR O SERVIDOR OU <CTRL+C> PARA SAIR SEM REINICIALIZAR"
reboot
